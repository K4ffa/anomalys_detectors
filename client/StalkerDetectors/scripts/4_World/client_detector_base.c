#define DETECTOR_RPC_MODE_CLIENT

enum StalkerDetectors
{
	BeepArtSound	= 12360,
    SendPosDir,
}

class SA_Detector_Base extends ItemBase
{
    protected ref OpenableBehaviour m_Openable;
	protected float m_soundTime;
	protected float m_curSoundTime;

    void SA_Detector_Base()
	{
		m_Openable = new OpenableBehaviour(false);
		
		RegisterNetSyncVariableBool("m_Openable.m_IsOpened");
		RegisterNetSyncVariableFloat("m_soundTime");
		
		UpdateVisualState();
	}
    
    #ifdef DETECTOR_RPC_MODE_CLIENT
    override void OnWork( float consumed_energy )
	{
        if(GetGame().IsClient() || !GetGame().IsMultiplayer())
		{
            array<ref Param> m_params = new array<ref Param>;
            vector m_positionRPC    = GetPosition();
            vector m_orientationRPC = GetOrientation();

            Param1<vector> p1 = new Param1<vector>(m_positionRPC);
            Param1<float> p2 = new Param1<float>(m_orientationRPC[0]);
            m_params.Insert(p1);
            m_params.Insert(p2);

            GetGame().RPC( this, StalkerDetectors.SendPosDir, m_params, true );
        }
    }
    #endif
	
	//////////////////////////////////////////////////////////////////////////////////////////////
	// Sounds && Visuals
	//////////////////////////////////////////////////////////////////////////////////////////////
	override void OnVariablesSynchronized()
	{
		super.OnVariablesSynchronized();
		
		UpdateVisualState();
		
		if(GetGame().IsClient() || !GetGame().IsMultiplayer())
		{
			if ( m_soundTime != m_curSoundTime )
			{
				SoundController(m_soundTime);
			}
		}
	}

    override void OnRPC(PlayerIdentity sender, int rpc_type,ParamsReadContext  ctx) 
	{
		super.OnRPC(sender, rpc_type, ctx);

		switch(rpc_type)
		{
			case StalkerDetectors.BeepArtSound:
                BeepArtSound()
			break;
		}
	}
	
	void SoundController(float time)
	{
		DisableBeepSound();
		if ( time != 0 ) EnableBeepSound(time);
	}
	
	void EnableBeepSound(float time)
	{
		BeepSound();
		GetGame().GetCallQueue(CALL_CATEGORY_SYSTEM).CallLater(this.BeepSound, time*1000, true);
		m_curSoundTime = time;
	}
	
	void DisableBeepSound()
	{
		GetGame().GetCallQueue(CALL_CATEGORY_SYSTEM).Remove(this.BeepSound);
		m_curSoundTime = 0;
	}
	
	void BeepSound()
	{
		PlaySound("detector_beep", 5);
	}
	
	void BeepArtSound()
	{
		PlaySound("detector_beep_art", 5);
	}
	

    override void OnItemLocationChanged(EntityAI old_owner, EntityAI new_owner)
	{
		super.OnItemLocationChanged(old_owner, new_owner);
		
		if ( GetCompEM().IsWorking() )
		{
			GetCompEM().SwitchOff();
		}
        
        if ( IsOpen() )
		{
            Close();
        }
	}

	override void Open()
	{
		m_Openable.Open();
		SetSynchDirty();

		UpdateVisualState();
	}

	override void Close()
	{
		m_Openable.Close();
		SetSynchDirty();

		UpdateVisualState();
	}
	
	override bool IsOpen()
	{
		return m_Openable.IsOpened();
	}

	protected void UpdateVisualState()
	{
		if ( IsOpen() )
		{
			SetAnimationPhase("cover", 1.0);
		}
		else
		{
			SetAnimationPhase("cover", 0.0);
		}
	}

    // Inventory manipulation
	/*
	override void OnInventoryEnter(Man player)
	{
		super.OnInventoryEnter(player);
		Print("OnInventoryEnter");
	}
	
	override void OnInventoryExit(Man player)
	{
		super.OnInventoryExit(player);
		Print("OnInventoryExit");
	}
	*/
	// Inventory manipulation

	override void SetActions()
	{
		super.SetActions();
		
		AddAction(ActionTurnOnDetectorWhileInHands);
		AddAction(ActionTurnOffDetectorWhileInHands);
	}
}